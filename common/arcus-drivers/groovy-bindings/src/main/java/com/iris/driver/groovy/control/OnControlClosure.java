...
public class OnControlClosure extends Closure<Object> {
   private final EnvironmentBinding binding;

   public OnControlClosure(EnvironmentBinding binding) {
      super(binding);
   ...
   protected void doCall(String messageType, Closure<?> closure) {
      addHandler(messageType, closure);
   }
   ...
   protected void addHandler(String messageType, Closure<?> closure) {
      ControlProtocolEventMatcher matcher = new ControlProtocolEventMatcher();
