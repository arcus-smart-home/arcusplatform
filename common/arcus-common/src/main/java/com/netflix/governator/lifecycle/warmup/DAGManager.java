...
// Patched copy of com.netflix.governator:governator:1.12.15 DAGManager.
// TypeToken.isAssignableFrom(Type) was removed in Guava 20; replaced with
// the semantically identical isSupertypeOf(Type) for Guava 33 compatibility.
...
public class DAGManager
{
    ...
    private void addImplicitDependency(final Object parent, final Object child)
    {
        if ( (parent instanceof TypeLiteral<?>) && (child instanceof TypeLiteral<?>) && !parent.equals(child) )
        {
            ...
            if ( TypeToken.of(parentType.getType()).isSupertypeOf(childType.getType()) )
            {
