...
import com.iris.protocol.ipcd.definition.context.Signature;
import com.iris.protocol.ipcd.definition.reader.DefinitionReader;

public class ModelGenerator {
   public final static Logger logger = LoggerFactory.getLogger(ModelGenerator.class);
   ...
   public static void main(String[] args) {
      ...
      DefinitionReader bindingReader = new DefinitionReader();
      DefinitionContext context = bindingReader.readBindings(input);
            
      try {
         JavaProcessor processor = new JavaProcessor(arguments.template, output.getAbsolutePath());
         processor.createClasses(context);
      } catch (IOException e) {
         logger.error("Could not create ipcd bindings!");
         e.printStackTrace();
         System.exit(-1);
      }
   }
