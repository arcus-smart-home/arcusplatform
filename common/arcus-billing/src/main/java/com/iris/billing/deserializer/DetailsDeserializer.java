...
public class DetailsDeserializer extends AbstractRecurlyDeserializer<Details> {
   ...
   DetailsDeserializer(SAXParser saxParser) {
      super(saxParser, Details.Tags.TAG_NAME);
      details = new Details();
   }

   @Override
   protected Details getResult() {
      return details;
   }
   ...
   protected void onReturnFrom(AbstractRecurlyDeserializer<?> childDeserializer) {
      if (childDeserializer instanceof AccountDeserializer) {
         details.add((Account)childDeserializer.getResult());
      }
   }
