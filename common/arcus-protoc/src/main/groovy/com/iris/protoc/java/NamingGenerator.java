...
public class NamingGenerator extends JavaGenerator {
   ...
   private void generateBindingsList(
      String packageName, 
      String subpackage, 
      Set<String> bindings) throws Exception {
      ...
      try (Writer out = new BufferedWriter(new FileWriter(new File(pkgDir, "Zigbee" + type + "Naming.java")))) {
         com.github.jknack.handlebars.Template template = handlebars.compile("NamingGroup");
   ...
   public void generateConstantBindings(String opkg, Map<QualifiedName,ConstantsNode> consts, Set<String> bindings) throws Exception {
      for (Map.Entry<QualifiedName,ConstantsNode> entry : consts.entrySet()) {
         ...
         context.put("hasMessages", false);

         try (Writer out = new BufferedWriter(new FileWriter(new File(pkgDir, qname.getMessage() + "Naming.java")))) {
            com.github.jknack.handlebars.Template template = handlebars.compile("Naming");
   ...
   public void generateMessageBindings(ProtocGeneratorOptions options,
      ...
      try (Writer out = new BufferedWriter(new FileWriter(new File(pkgDir, clazz + "Naming.java")))) {
