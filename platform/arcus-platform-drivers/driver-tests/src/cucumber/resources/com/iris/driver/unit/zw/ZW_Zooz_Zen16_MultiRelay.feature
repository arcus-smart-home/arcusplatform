@ZWave @Zooz @switch
Feature: ZWave Zooz ZEN16 MultiRelay Driver Test

These scenarios test the functionality of the ZWave Zooz ZEN16 MultiRelay driver

    Background:
    Given the ZW_Zooz_Zen16_MultiRelay.driver has been initialized
    ...
    Then the driver should place a base:GetAttributesResponse message on the platform bus
        ...
        And the message's devadv:drivername attribute should be ZWZoozZen16MultiRelayDriver
...
############################################################
    ...
    @switch
    Scenario: Device reports relay 3 ON via multi-channel encapsulated switch binary report
        Given the capability swit:state.relay3 is OFF
        When the device response with multi_channel encapsulate
            And with parameter source 3
            And with parameter command_class 37
            And with parameter command 3
            And with parameter parameter1 -1
            And send to driver
        Then the capability swit:state.relay3 should be ON
            And the driver should place a base:ValueChange message on the platform bus

    @switch
    Scenario: Device reports relay 3 OFF via multi-channel encapsulated switch binary report
        Given the capability swit:state.relay3 is ON
        When the device response with multi_channel encapsulate
            And with parameter source 3
            And with parameter command_class 37
            And with parameter command 3
            And with parameter parameter1 0
            And send to driver
        Then the capability swit:state.relay3 should be OFF
            And the driver should place a base:ValueChange message on the platform bus
