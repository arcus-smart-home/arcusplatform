@ZWave @Schlage @lock @BE469
Feature: ZWave Schlage Keypad Deadbolt BE469 Driver Test

    These scenarios test the functionality of the ZWave Schlage Keypad Deadbolt driver.
    
    Background:
        Given the ZW_Schlage_Touch_Deadbolt_2_6.driver has been initialized
...
############################################################
    ...
    Scenario: Door Lock reports alarm state of Replace Battery Soon
        Given the capability devpow:battery is 40
        When the device response with alarm report 
            And with parameter notificationType 8
            And with parameter event 10
            And send to driver
        Then the platform attribute devpow:battery should change to 25
            And the driver should place a base:ValueChange message on the platform bus
        Then both busses should be empty

    Scenario: Door Lock reports alarm state of Replace Battery Soon
        Given the capability devpow:battery is 20
        When the device response with alarm report 
            And with parameter notificationType 8
            And with parameter event 10
            And send to driver
        Then the platform attribute devpow:battery should be 20

    Scenario: Door Lock reports alarm state of Replace Battery Now
        Given the capability devpow:battery is 40
        When the device response with alarm report 
            And with parameter notificationType 8
            And with parameter event 11
    ...
    Scenario: Door Lock reports alarm state of System Hardware Failure
        When the device response with alarm report 
            And with parameter notificationType 9
            And with parameter event 1
            And send to driver
        Then both busses should be empty

    Scenario: Door Lock reports alarm state of Factory Reset
        When the device response with alarm report 
            And with parameter notificationType 9
            And with parameter event 5
            And send to driver
        Then both busses should be empty

    Scenario: TransactionTimeoutEvent fires while locking
        Given the capability doorlock:lockstate is LOCKING
        When the event TransactionTimeoutEvent triggers
        Then the platform attribute doorlock:lockstate should be LOCKING
            And the capability devadv:errors should be [WARN_JAM:Door lock may be jammed]

    Scenario: TransactionTimeoutEvent fires while unlocking
        Given the capability doorlock:lockstate is UNLOCKING
        When the event TransactionTimeoutEvent triggers
        Then the platform attribute doorlock:lockstate should be UNLOCKING
            And the capability devadv:errors should be [WARN_JAM:Door lock may be jammed]
