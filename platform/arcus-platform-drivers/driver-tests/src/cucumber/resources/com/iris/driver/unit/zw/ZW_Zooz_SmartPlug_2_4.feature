@ZWave @Zooz @switch
Feature: ZWave Zooz Smart Plug Driver Test

These scenarios test the functionality of the ZWave Zooz Smart Plug driver

    Background:
    Given the ZW_Zooz_SmartPlug_2_4.driver has been initialized
    ...
    Then the driver should place a base:GetAttributesResponse message on the platform bus
        ...
        And the message's devadv:drivername attribute should be ZWZoozSmartPlugDriver 
...
############################################################
    ...
    Scenario: Make sure the offline timeout is set each time the device Connects
        ...
        Then the driver should poll switch_binary.get every 60 minutes
            # config LED mode to 'show on change'
            And the driver should send configuration set
                And with parameter param 27
                And with parameter size 1
                And with parameter val1 1
            #config Overload Protection to enabled
            And the driver should send configuration set
                And with parameter param 20
                And with parameter size 1
                And with parameter val1 1
            # config Switch State after power-cycle to 'previous state'
            And the driver should send configuration set
                And with parameter param 21
                And with parameter size 1
                And with parameter val1 0
            # config Switch State Notification mode to report Local and Remote changes
            And the driver should send configuration set
                And with parameter param 24
                And with parameter size 1
                And with parameter val1 1
            # config Watt Delta Threshold 5 Watts
            And the driver should send configuration set
                And with parameter param 151
                And with parameter size 2
                And with parameter val1 0
                And with parameter val2 5
            # config Watt % Delta Threshold to 10%
            And the driver should send configuration set
                And with parameter param 152
                And with parameter size 1
                And with parameter val1 10
            # config Watt Reporting Freq to 1 hour (3600 seconds)
            And the driver should send configuration set
                And with parameter param 171
                And with parameter size 4
                And with parameter val1 0
                And with parameter val2 0
                And with parameter val3 14
                And with parameter val4 16
            # config KWh Reporting Freq to 1 hour (3600 seconds)
            And the driver should send configuration set
                And with parameter param 172
                And with parameter size 4
                And with parameter val1 0
                And with parameter val2 0
                And with parameter val3 14
