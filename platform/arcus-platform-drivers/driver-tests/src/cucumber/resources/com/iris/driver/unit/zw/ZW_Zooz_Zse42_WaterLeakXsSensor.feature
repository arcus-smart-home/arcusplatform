@ZWave @Zooz @leak
Feature: ZWave Zooz ZSE42 Water Leak XS Sensor Driver Test

These scenarios test the functionality of the ZWave Zooz ZSE42 Water Leak XS Sensor driver

    Background:
    Given the ZW_Zooz_Zse42_WaterLeakXsSensor.driver has been initialized

    @basic
    Scenario: Driver reports capabilities to platform.
    When a base:GetAttributes command is placed on the platform bus
    Then the driver should place a base:GetAttributesResponse message on the platform bus
        And the message's base:caps attribute list should be ['base', 'dev', 'devadv', 'devpow', 'devconn', 'devsettings', 'leakh2o']
        And the message's dev:devtypehint attribute should be Water Leak
        And the message's devadv:drivername attribute should be ZWZoozZse42WaterLeakXsSensorDriver
...
############################################################
    ...
    @leak
    Scenario Outline: Device reports an invalid state change via a 'basic' report
        Given the capability leakh2o:state is <prev_state>
        When the device response with basic report
            And with parameter value <value>
            And send to driver
