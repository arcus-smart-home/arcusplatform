@ZWave @Jasco @Fan
Feature: GE/Jasco In-Wall Fan Controller
  
  Validates message handling for the GE/Jasco in-wall fan controller driver.

    Background: 
        Given the ZW_Jasco_InWallFanController.driver has been initialized

    @basic
    Scenario: Driver reports capabilities to platform.
        When a base:GetAttributes command is placed on the platform bus
        Then the driver should place a base:GetAttributesResponse message on the platform bus
            And the message's base:caps attribute list should be ['base', 'dev', 'devadv', 'devpow', 'devconn', 'fan', 'swit', 'indicator']
            And the message's dev:devtypehint attribute should be Fan Control
            And the message's devadv:drivername attribute should be ZWJascoInWallFanControllerDriver
    ...
    @basic @added
     Scenario: Device associated
        ...
        Then the driver should send configuration set
            And with parameter param 0
            And with parameter size 0x81
