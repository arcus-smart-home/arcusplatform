@ZWave @Generic @Thermostat
Feature: ZWave Generic Thermostat Driver Test

	These scenarios test the functionality of the ZWave Generic Thermostat driver.
	
	Background:
		Given the ZW_ZZWaveGenericThermostat_2_12.driver has been initialized
	
	Scenario: Driver reports capabilities to platform. 
	When a base:GetAttributes command is placed on the platform bus
	Then the driver should place a base:GetAttributesResponse message on the platform bus
		And the message's base:caps attribute list should be ['base', 'dev', 'devadv', 'devpow', 'devconn', 'temp','therm']
		And the message's dev:devtypehint attribute should be Thermostat
		And the message's devadv:drivername attribute should be ZZWaveGenericThermostat 
		...
		And the message's therm:supportsAuto attribute should be false
		And the message's therm:minsetpoint attribute should be 4.4
		And the message's therm:maxsetpoint attribute should be 37.2
...
############################################################
	...
	Scenario Outline: Device reports a new fan mode
		...
		Then the driver should place a base:ValueChange message on the platform bus
			And the platform attribute therm:fanmode should change to <fanmode>
