...
//   public static final String PRGREC_TRIM = "DELETE FROM " + PURGE_RECORDINGS_TABLE + " WHERE deletetime=? AND partitionid=? AND recordingid=?";
	...
	@Inject
	public PurgeRecordingV2Table(String ts, CqlSession session) {
		super(ts, session);
	}
	...
	protected PurgeRecord buildEntity(Row row) {
		...
		return new PurgeRecord(row.isNull(COL_DELETETIME) ? null : Date.from(row.getInstant(COL_DELETETIME)), row.getInt(COL_PARTITIONID), row.getUuid(COL_RECORDINGID), row.getUuid(COL_PLACEID), storageStr, true, purgePreview);
	}
