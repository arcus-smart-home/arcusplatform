...
public class TestPanicAlarm_Alert extends PanicAlarmTestCase {
	Model keypad;
	Model panicRule;
	...
	public void bind() {
		...
		// need to pre-stage the system to be in alert since there is no persistent trigger
		AlarmModel.setAlertState(PanicAlarm.NAME, model, AlarmCapability.ALERTSTATE_ALERT);
		AlarmModel.setDevices(PanicAlarm.NAME, model, addressesOf(keypad, panicRule));
		AlarmModel.setActiveDevices(PanicAlarm.NAME, model, addressesOf(keypad, panicRule));
		AlarmModel.setTriggeredDevices(PanicAlarm.NAME, model, addressesOf());
		AlarmModel.setOfflineDevices(PanicAlarm.NAME, model, addressesOf());
	...
	@Test
	public void testRemoveKeyPadThenRuleThenCancel() {
		removeModel(keypad);
	...
	@Test
	public void testRemoveRuleThenKeyPadThenCancel() {
		removeModel(panicRule);
