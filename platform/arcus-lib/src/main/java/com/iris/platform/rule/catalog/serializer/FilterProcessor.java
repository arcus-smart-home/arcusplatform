...
public class FilterProcessor extends ConditionsContainerProcessor {
   public static final String TAG = "filter";
   public static final String ATTR_REF = "ref";
   
   private ReferenceFilterConfig filter;

   protected FilterProcessor(Validator validator) {
      super(validator);
   }
   ...
   protected void setCondition(ConditionConfig condition) {
      ...
      this.filter.setCondition(condition);
   }
   ...
   private void parseFilter(Attributes attributes) {
      filter = new ReferenceFilterConfig();
      filter.setReference(getValue(ATTR_REF, attributes));
   }
}
