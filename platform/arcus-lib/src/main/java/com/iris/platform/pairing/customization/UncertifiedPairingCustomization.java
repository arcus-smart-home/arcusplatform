...
class UncertifiedPairingCustomization extends PairingCustomization {
	private static final Logger logger = LoggerFactory.getLogger(UncertifiedPairingCustomization.class);

	UncertifiedPairingCustomization(
			String action,
			String id,
	...
	protected boolean apply(Place place, Model device) {
		...
		if(!productId.isEmpty()) {
			...
			if(entry != null) {
				return entry.getCert() != Cert.WORKS;
			}
		}
		return false;
	}
