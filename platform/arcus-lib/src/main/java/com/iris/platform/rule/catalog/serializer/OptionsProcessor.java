...
public class OptionsProcessor extends BaseCatalogProcessor {

   public static final String TAG = "options";

   private List<Option> options = new ArrayList<>();

   protected OptionsProcessor(Validator v) {
      super(v);
   }
   ...
   public TagProcessor getHandler(String qName, Attributes attributes) {
      if(OptionProcessor.TAG.equals(qName)) {
         return new OptionProcessor(getValidator());
      }
   ...
   public void exitChildTag(String qName, TagProcessor handler) {
      if(OptionProcessor.TAG.equals(qName)) {
         options.add(((OptionProcessor) handler).getOption());
      }
   }
}
