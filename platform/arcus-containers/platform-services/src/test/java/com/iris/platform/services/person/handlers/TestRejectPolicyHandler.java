...
public class TestRejectPolicyHandler extends IrisMockTestCase {
   ...
   // unit under test
   @Inject
   private AcceptPolicyHandler handler;
   ...
   public void stageFixtures() throws Exception {
      ...
      person.setTermsAgreed(null);
      person.setPrivacyPolicyAgreed(null);
   ...
   @Test
   public void testAcceptPolicyPrivacy() throws Exception {
      EasyMock
      ...
      MessageBody request = 
            PersonCapability.AcceptPolicyRequest.builder()
               .withType(PersonCapability.AcceptPolicyRequest.TYPE_PRIVACY)
               .build();

      MessageBody response = handler.handleRequest(person, request(request));
      
      assertEquals(MessageBody.emptyMessage(), response);
      assertNotNull(person.getPrivacyPolicyAgreed());
      assertEquals(null, person.getTermsAgreed());
      verify();
   }
   ...
   public void testRejectPolicyTerms() throws Exception {
      ...
      MessageBody request = 
            PersonCapability.AcceptPolicyRequest.builder()
               .withType(PersonCapability.AcceptPolicyRequest.TYPE_TERMS)
               .build();

      MessageBody response = handler.handleRequest(person, request(request));
      
      assertEquals(MessageBody.emptyMessage(), response);
      assertEquals(null, person.getPrivacyPolicyAgreed());
      assertNotNull(person.getTermsAgreed());
      verify();
   }
