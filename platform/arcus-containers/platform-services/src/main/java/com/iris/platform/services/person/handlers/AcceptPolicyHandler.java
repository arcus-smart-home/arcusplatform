...
@Singleton
public class AcceptPolicyHandler implements ContextualRequestMessageHandler<Person> {

   private final PersonDAO personDao;

   @Inject
   public AcceptPolicyHandler(
         PersonDAO personDao,
   ...
   public String getMessageType() {
      return PersonCapability.AcceptPolicyRequest.NAME;
   }
   ...
   public MessageBody handleRequest(Person context, PlatformMessage msg) {
      ...
      String policyType = PersonCapability.AcceptPolicyRequest.getType(body);
      if(PersonCapability.AcceptPolicyRequest.TYPE_PRIVACY.equals(policyType)) {
    	  context.setPrivacyPolicyAgreed(new Date());
      }else if(PersonCapability.AcceptPolicyRequest.TYPE_TERMS.equals(policyType)) {
    	  context.setTermsAgreed(new Date());
      }else {
   ...
   private void validate(MessageBody body) {
      Errors.assertRequiredParam(PersonCapability.AcceptPolicyRequest.getType(body), "type");
   }
