...
@Singleton
public class ListAvailablePlacesHandler implements ContextualRequestMessageHandler<Person> {
   ...
   public String getMessageType() {
      return PersonCapability.ListAvailablePlacesRequest.NAME;
   }

   @Override
   public MessageBody handleRequest(Person context, PlatformMessage msg) {
      List<Map<String,Object>> places = personPlaceAssocDao.listPlaceAccessForPerson(context.getId())
            .stream().map(PlaceAccessDescriptor::toMap).collect(Collectors.toList());

      return PersonCapability.ListAvailablePlacesResponse.builder()
            .withPlaces(places)
            .build();
   }
}
