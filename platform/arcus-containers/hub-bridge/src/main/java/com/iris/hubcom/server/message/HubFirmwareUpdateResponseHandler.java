...
@Singleton
public class HubFirmwareUpdateResponseHandler extends DirectMessageHandler {
   private static final Logger logger = LoggerFactory.getLogger(HubFirmwareUpdateResponseHandler.class);   
   private final HubRegistrationMessageHandlerAdaptor hubRegistrationMsgAdaptor;
   
   @Inject
   public HubFirmwareUpdateResponseHandler(
   ...
   public String supportsMessageType() {
		return HubAdvancedCapability.FirmwareUpdateResponse.NAME;
   }
	
   @Override
   protected void doHandle(Session session, PlatformMessage msg) {
      hubRegistrationMsgAdaptor.handleFirmwareUpdateResponse(((HubSession)session).getHubId(), msg);		
   }
