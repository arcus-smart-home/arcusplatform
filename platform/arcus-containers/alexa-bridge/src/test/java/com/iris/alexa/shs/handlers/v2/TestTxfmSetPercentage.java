...
public class TestTxfmSetPercentage {

   private Header setPercentageHeader;

   @Before
   public void setup() {
      setPercentageHeader = Header.v2(IrisUUID.randomUUID().toString(), "SetPercentageRequest", "Alexa.ConnectedHome.Control");
   }

   @Test
   public void testSetPercentageLight() {
      Appliance app = new Appliance();
      app.setApplianceId(Address.platformDriverAddress(IrisUUID.randomUUID()).getRepresentation());

      SetPercentageRequest payload = new SetPercentageRequest();
      payload.setAccessToken("token");
      payload.setAppliance(app);
      payload.setPercentageState(new DoubleValue(10.0));

      AlexaMessage msg = new AlexaMessage(setPercentageHeader, payload);
      ...
      ShsAssertions.assertExecuteRequest(
         ...
         ImmutableMap.of(AlexaInterfaces.BrightnessController.PROP_BRIGHTNESS, 10),
         null,
         false
      );
   }
   ...
   @Test
   public void testSetPercentageFan() {
      Appliance app = new Appliance();
      app.setApplianceId(Address.platformDriverAddress(IrisUUID.randomUUID()).getRepresentation());
      ...
      payload.setAccessToken("token");
      payload.setAppliance(app);
      payload.setPercentageState(new DoubleValue(25.0));
      ...
      ShsAssertions.assertExecuteRequest(
         ...
         ImmutableMap.of(AlexaInterfaces.PercentageController.PROP_PERCENTAGE, 25),
         null,
         false
      );
   }
   ...
   public void testConfirmation() {
      ...
      ShsAssertions.assertCommonResponseHeader(msg, response, "SetPercentageConfirmation", "2");
      assertTrue(response.getPayload() instanceof SetPercentageConfirmation);
   }
}
