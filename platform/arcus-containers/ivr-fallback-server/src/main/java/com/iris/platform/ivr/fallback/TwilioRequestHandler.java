...
@Singleton
@HttpGet("/ivr/request?*")
public class TwilioRequestHandler extends RequestHandlerImpl {
   ...
   private static final Counter INBOUND_REQUEST_COUNTER = METRICS.counter("request.inbound.count");

   @Inject
   public TwilioRequestHandler(AlwaysAllow alwaysAllow) {
      super(alwaysAllow, new FallbackResponder("request.xml", INBOUND_REQUEST_COUNTER));
   }
