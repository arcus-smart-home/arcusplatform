 ...
 *
 */
public abstract class SubsystemTestCase<M extends SubsystemModel> extends IrisMockTestCase {
   ...
   @Provides @Singleton
   public DefinitionRegistry definitionRegistry() {
      return ClasspathDefinitionRegistry.instance();
   }
   ...
   public void setUp() throws Exception {
      ...
      placeModel = addModel(ModelFixtures.createPlaceAttributes());
      placeId = (UUID)placeModel.getAddress().getId();
      accountModel = addModel(ModelFixtures.createAccountAttributes());
      ...
      EasyMock
         ...
         .anyTimes();
      EasyMock
         .expect(context.request(EasyMock.capture(requestAddresses), EasyMock.capture(requests), EasyMock.anyInt()))
         .andAnswer(new IAnswer<String>() {
            @Override
            public String answer() throws Throwable {
               String correlationId = UUID.randomUUID().toString();
   ...
   protected abstract Subsystem<M> subsystem();
