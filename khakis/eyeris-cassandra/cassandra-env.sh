...
#JVM_OPTS="$JVM_OPTS -Xmn${HEAP_NEWSIZE}" # we should not set new gen size for g1gc
...
# GC tuning options
if [ "${CASSANDRA_USE_G1GC}" = "YES"  ] ; then 
    JVM_OPTS="$JVM_OPTS -XX:+UseG1GC -XX:G1RSetUpdatingPauseTimePercent=5 -XX:MaxGCPauseMillis=250"
else
